...
* `sort(items, algorithm='powersort', comparator=nil, ascending=true)` - sort a mutable list in place. `powersort` is the stable default; `quicksort` is an unstable, iterative three-way quicksort. With no comparator, elements are ordered using `<`, including a user type's overloaded `operator<`. A comparator is a function `(a, b) -> bool` that returns true when `a` precedes `b`; `ascending=false` reverses that relation while preserving equal-element order under Powersort.
* `sorted(items, algorithm='powersort', comparator=nil, ascending=true)` - return a shallow sorted copy, leaving the input list and its element objects unchanged. It accepts the same algorithms and ordering options as `sort`.
