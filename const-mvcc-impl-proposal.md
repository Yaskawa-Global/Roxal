...
`move()` transfers ownership and invalidates the source. Since the moved value has sole ownership (enforced at runtime), no snapshot can reference it through a mutable alias — so no version-saving overhead.

### Actor Method Parameters (Implicit Const)
Actor method parameters are implicitly const per the spec. This uses the same `T → const T` mechanism: shallow-clone the argument, set ConstMask. This replaces the current eager deep-copy (`clone()`) on actor calls with a much cheaper shallow clone.
