    ...
    : typeHandle(eventType)
{
    ...
    if (isEvent(eventType)) {
        ObjEventType* typeObj = asEvent(eventType);
...
ObjEventType* ObjSignal::ensureChangeEvent()
{
    ...
    signal->addValueChangedCallback([eventWeak](TimePoint t, ptr<df::Signal>, const Value&){
        if (!eventWeak.isAlive())
            return;
        ObjEventType* ev = asEvent(eventWeak);
...
std::string roxal::objEventTypeToString(const ObjEventType* ev)
{
    ...
    if (name.empty())
        return std::string("<event type>");
    return std::string("<event type ") + name + ">";
}
...
std::string roxal::objEventInstanceToString(const ObjEventInstance* ev)
{
    ...
    std::string typeName;
    if (ev->typeHandle.isAlive() && isEvent(ev->typeHandle)) {
        asEvent(ev->typeHandle)->name.toUTF8String(typeName);
    }
    if (typeName.empty())
        typeName = "event";
...
std::string roxal::objToString(const Value& v)
{
    switch(objType(v)) {
        ...
        case ObjType::EventType: {
            return objEventTypeToString(asEvent(v));
        }
