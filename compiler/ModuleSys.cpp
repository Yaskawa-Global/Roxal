...
std::string typeHelpString(ObjObjectType* type)
{
    std::ostringstream out;
    out << describeTypeKind(type) << " " << toUTF8StdString(type->name) << "\n";
    out << "Properties:\n";
...
Value ModuleSys::help_builtin(VM& vm, ArgsView args)
{
    ...
    if (isObjectType(target)) {
        return Value::stringVal(toUnicodeString(typeHelpString(asObjectType(target))));
    }
