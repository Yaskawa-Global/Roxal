...
bool VM::callValue(const Value& callee, const CallSpec& callSpec)
{
    ...
    if (signalArg && callee.isObj() && objType(callee) == ObjType::Native && asNative(callee)->funcType) {
        ...
        if (nativeObj->funcType) {
            ...
            const auto& funcType = nativeObj->funcType->func.value();
            for (size_t pi = 0; pi < paramPositions.size(); ++pi) {
                int argIndex = paramPositions[pi];
                if (argIndex == -1) continue;
                Value arg = peek(callSpec.argCount - 1 - argIndex);
            ...
            for (int pi = 0; pi < callSpec.argCount; ++pi) {
                Value arg = peek(callSpec.argCount - 1 - pi);
                std::string pname = std::to_string(pi);
                paramNames.push_back(pname);
                if (isSignal(arg))
                    sigArgs.push_back(asSignal(arg)->signal);
                else {
        ...
        df::FuncNode::NativeFunc nf = [fn=nativeObj->function](const df::Values& vals)->df::Values {
            std::vector<Value> args(vals.begin(), vals.end());
            Value res = (VM::instance().*fn)(args.size(), args.data());
            return { res };
        };

        auto node = roxal::make_ptr<df::FuncNode>("native", nf, paramNames, constArgs, sigArgs);
        node->addToEngine();
        auto outputs = node->outputs();
        dataflowEngine->evaluate();
        popN(callSpec.argCount + 1);
        if (outputs.size() == 1) {
            push(objVal(signalVal(outputs[0])));
        } else if (outputs.empty()) {
    ...
    if (signalArg && callee.isObj() && objType(callee) == ObjType::BoundNative && asBoundNative(callee)->funcType) {
        ...
        if (bound->funcType) {
            ...
            const auto& funcType = bound->funcType->func.value();
            for (size_t pi = 0; pi < paramPositions.size(); ++pi) {
                int argIndex = paramPositions[pi];
                if (argIndex == -1) continue;
                Value arg = peek(callSpec.argCount - 1 - argIndex);
        ...
        df::FuncNode::NativeFunc nf = [fn=bound->function, receiver=bound->receiver](const df::Values& vals)->df::Values {
