...
void LazyModuleRegistry::doLoad(VM& vm, const std::string& name)
{
    ...
    if (instance->hasModuleScript()) {
        ...
        //    A module may specify its own script path (e.g. ui -> "ui/ui.rox").
        //    Otherwise convert dots to path separators for nested modules
        //    (e.g. "ai.nn" → "ai/nn.rox").
        std::string roxFile = instance->moduleScriptPath();
        if (roxFile.empty()) {
            roxFile = name;
